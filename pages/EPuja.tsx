...
import { Video, Package, Globe, Laptop, Gift, Truck } from 'lucide-react';
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="bg-stone-900 text-saffron-50 py-24 px-4 relative overflow-hidden">
        ...
        >
          ...
          <h1 className="text-5xl md:text-6xl font-serif font-bold mb-8 leading-tight">E-Puja for Global Devotees</h1>
          <p className="text-xl text-stone-300 max-w-2xl mx-auto font-light leading-relaxed">
            Don't let distance disconnect you from your roots. Experience authentic Vedic rituals from the comfort of your home abroad.
          </p>
        </motion.div>
      </div>
      ...
      <div className="container mx-auto px-4 py-20">
        ...
        <div className="grid md:grid-cols-4 gap-8 mb-24 relative">
           ...
           {[
             { title: "Consultation", desc: "Video call with Panditji to decide the Puja.", icon: <Video size={20} /> },
             { title: "Sankalp", desc: "Live Sankalp using your Name, Gotra & Location.", icon: <Laptop size={20} /> },
             { title: "Live Streaming", desc: "Watch the Havan via Zoom/Google Meet.", icon: <Globe size={20} /> },
             { title: "Prasad Shipping", desc: "Raksha Sutra & Vibhuti couriered to you.", icon: <Truck size={20} /> },
           ].map((step, i) => (
             <motion.div 
               key={i} 
        ...
        <div className="text-center mb-12">
            <h2 className="text-3xl font-serif font-bold text-saffron-900">Popular NRI Packages</h2>
            <p className="text-stone-500 mt-2">Tailored for your convenience and time zone.</p>
        </div>
        
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {[
            { name: "Birthday Ayush Havan", price: "$151 USD", icon: "🎂", features: ["Longevity Prayers", "Cake Cutting Timing", "Digital Photos"] },
            { name: "Graha Shanti", price: "$201 USD", icon: "✈️", sub: "Visa & Immigration", features: ["Visa Problem Solving", "Career Stability", "Legal Victory"] },
            { name: "Annual Family Welfare", price: "$301 USD", icon: "🏠", features: ["Complete Protection", "Health & Wealth", "Ancestral Peace"] },
          ].map((pkg, i) => (
            <motion.div 
              key={i} 
            ...
            >
              <div className="text-5xl mb-6 bg-white w-24 h-24 mx-auto rounded-full flex items-center justify-center shadow-sm">{pkg.icon}</div>
